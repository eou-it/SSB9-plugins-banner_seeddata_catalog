...
/**
 * Employee CALBHR Person ID DML for Leave by Employee
 */
public class EmployeeCalbhrPersonIDDML {
    def bannerid
    def pebempl_pidm
    ...
    def pebempl_pers_appt_id
    def pebempl_pers_participant_id
    ...
    public EmployeeCalbhrPersonIDDML(InputData connectInfo, Sql conn, Connection connectCall) {
        this.conn = conn
    ...
    public EmployeeCalbhrPersonIDDML(InputData connectInfo, Sql conn, Connection connectCall, xmlData) {
        this.conn = conn
        this.connectInfo = connectInfo
        this.connectCall = connectCall
        this.xmlData = xmlData
        parseXmlData()
        processCalbhrEmployee()
    }
    ...
    def parseXmlData() {
        ...
        this.pebempl_pers_appt_id = pebempl.PEBEMPL_PERS_APPT_ID.text()
        this.pebempl_pers_participant_id = pebempl.PEBEMPL_PERS_PARTICIPANT_ID.text()
    ...
    def processCalbhrEmployee() {
        ...
        try {
            ...
            if (connectInfo.showErrors) {
                println "Could not select ID in CalbhrEmployeePersonIDDML, ${this.bannerid.text()} from SPRIDEN. $e.message"
            }
        }
        ...
        if (pidm) {
            try {
                ...
                String API = "{call pv_employee.p_create(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}"
                ...
                insertCall.setString(54, this.pebempl_pers_participant_id)
                insertCall.setString(55, this.pebempl_pers_appt_id)

                if (!this.pebempl_1095tx_consent_ind) {
                    insertCall.setString(56, "N")
                } else {
                    insertCall.setString(56, this.pebempl_1095tx_consent_ind)
                }

                if ((this.pebempl_1095tx_consent_date == "") || (this.pebempl_1095tx_consent_date == null) ||
                        (!this.pebempl_1095tx_consent_date)) {
                    insertCall.setNull(57, java.sql.Types.DATE)
                } else {
                    ddate = new ColumnDateValue(this.pebempl_1095tx_consent_date)
                    unfDate = ddate.formatJavaDate()
                    formatter = new SimpleDateFormat("yyyy-MM-dd");
                    sqlDate = new java.sql.Date(formatter.parse(unfDate).getTime());
                    insertCall.setDate(57, sqlDate)
                }
                insertCall.setString(58, this.pebempl_1095tx_consent_user_id)
                ...
                if (connectInfo.showErrors) {
                    println "Insert PEBEMPL ${this.bannerid} }"
                    println "Problem executing insert for table PEBEMPL from EmployeeCalbhrPersonIDDML.groovy: $e.message"
                }
            }
        }
    }
    ...
    def deleteData() {
        ...
        deleteData("PHRLRRQ", "delete from phrlrrq where phrlrrq_member_pidm = ?")
        deleteData("PHRLRRQ", "delete from phrlrrq where phrlrrq_pidm = ?")
    ...
    def deleteData(String tableName, String sql) {
        try {
            ...
            if (connectInfo.showErrors) {
                println "Problem executing ${tableName} delete for ${connectInfo.savePidm} from EmployeeCalbhrPersonIDDML.groovy: $e.message"
                println "${sql}"
            }
        }
    }
