...
/**
 * Created by apoliski on 8/21/2019.
 */
class EmployeeCalbhrJobAssignmentDetailDML {
    def bannerid
    ...
    def nbrjobs_pers_appt_id
    def nbrjobs_pers_pay_rate_over
    def nbrjobs_pers_pay_rate_type
    def nbrjobs_pers_sched_days
    def nbrjobs_pers_sched_hrs_week
    def nbrjobs_pers_special_only_ind
    def nbrjobs_scls_base_hrs
    def nbrjobs_scls_code
    def nbrjobs_stin_code
    ...
    public EmployeeCalbhrJobAssignmentDetailDML(InputData connectInfo, Sql conn, Connection connectCall) {
    ...
    public EmployeeCalbhrJobAssignmentDetailDML(InputData connectInfo, Sql conn, Connection connectCall, xmlData) {
        ...
        parseXmlData()
        processEmployeeCalbhrJobAssignmentDetail()
    }
    def parseXmlData() {
        ...
        this.nbrjobs_pers_appt_id = nbrjobs.NBRJOBS_PERS_APPT_ID.text()
        this.nbrjobs_pers_pay_rate_over = nbrjobs.NBRJOBS_PERS_PAY_RATE_OVER.text()
        this.nbrjobs_pers_pay_rate_type = nbrjobs.NBRJOBS_PERS_PAY_RATE_TYPE.text()
        this.nbrjobs_pers_sched_days = nbrjobs.NBRJOBS_PERS_SCHED_DAYS.text()
        this.nbrjobs_pers_sched_hrs_week = nbrjobs.NBRJOBS_PERS_SCHED_HRS_WEEK.text()
        this.nbrjobs_pers_special_only_ind = nbrjobs.NBRJOBS_PERS_SPECIAL_ONLY_IND.text()
        this.nbrjobs_scls_base_hrs = nbrjobs.NBRJOBS_SCLS_BASE_HRS.text()
        this.nbrjobs_scls_code = nbrjobs.NBRJOBS_SCLS_CODE.text()
        this.nbrjobs_stin_code = nbrjobs.NBRJOBS_STIN_CODE.text()
    }
    def processEmployeeCalbhrJobAssignmentDetail() {
        PIDM = null
        ...
        try {
            ...
            if (connectInfo.showErrors) {
                println "Could not select ID in EmployeeCalbhrJobAssignmentDetailDML,  ${this.bannerid.text()}  from SPRIDEN. $e.message"
            }
        }
        if (this.supervisor_bannerid) {
            ...
            try {
                ...
                if (connectInfo.showErrors) {
                    println "Could not select Supervisor ID in EmployeeCalbhrJobAssignmentDetailDML,  ${this.supervisor_bannerid.text()}  from SPRIDEN. $e.message"
                }
            }
        }
        ...
        if (PIDM) {
            ...
            try {
                ...
                if (connectInfo.showErrors) {
                    println "${findRow}"
                    println "Problem with select for table NBRJOBS from EmployeeCalbhrJobAssignmentDetailDML.groovy: $e.message"
                }
            }
            if (!findY) {
                try {
                    ...
                    */
                    ...
                    String API = "{call nv_job_detail.p_create(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}"
                    ...
                    insertCall.setString(51, this.nbrjobs_scls_code)
                    insertCall.setString(52, this.nbrjobs_stin_code)
                    insertCall.setString(53, this.nbrjobs_pers_appt_id)
                    insertCall.setString(54, this.nbrjobs_pers_pay_rate_type)
                    if ((this.nbrjobs_pers_sched_days == "") || (this.nbrjobs_pers_sched_days == null) ||
                            (!this.nbrjobs_pers_sched_days)) {
                        insertCall.setNull(55, java.sql.Types.DOUBLE)
                    } else {
                        insertCall.setDouble(55, this.nbrjobs_pers_sched_days.toDouble())
                    }
                    if ((this.nbrjobs_scls_base_hrs == "") || (this.nbrjobs_scls_base_hrs == null) ||
                            (!this.nbrjobs_scls_base_hrs)) {
                        insertCall.setNull(56, java.sql.Types.DOUBLE)
                    } else {
                        insertCall.setDouble(56, this.nbrjobs_scls_base_hrs.toDouble())
                    }
                    if ((this.nbrjobs_pers_sched_hrs_week == "") || (this.nbrjobs_pers_sched_hrs_week == null) ||
                            (!this.nbrjobs_scls_base_hrs)) {
                        insertCall.setNull(57, java.sql.Types.DOUBLE)
                    } else {
                        insertCall.setDouble(57, this.nbrjobs_pers_sched_hrs_week.toDouble())
                    }

                    insertCall.setString(58, this.nbrjobs_pers_special_only_ind)

                    if ((this.nbrjobs_pers_pay_rate_over == "") || (this.nbrjobs_pers_pay_rate_over == null) ||
                            (!this.nbrjobs_pers_pay_rate_over)) {
                        insertCall.setNull(59, java.sql.Types.DOUBLE)
                    } else {
                        insertCall.setDouble(59, this.nbrjobs_pers_pay_rate_over.toDouble())
                    }
                    insertCall.registerOutParameter(60, java.sql.Types.ROWID)
                    insertCall.registerOutParameter(61, java.sql.Types.VARCHAR)
                    ...
                    try {
                        ...
                        if (connectInfo.showErrors) {
                            ...
                            println "Problem executing insert for table NBRJOBS from EmployeeCalbhrJobAssignmentDetailDML.groovy: $e.message"
                        }
                    }
                    finally {
                        insertCall.close()
                    ...
                    if (this.nbrjobs_lcat_code) {
                       ...
                       try {
                            ...
                            connectInfo.tableUpdate("PTRINST", 0, 0, 0, 1, 0)
                            if (connectInfo.showErrors) {
                                println "Problem executing update PTRINST to Leave By Employee in EmployeeCalbhrLeaveByJobPersonIDDML.groovy: $e.message"
                            }
                        }
                        conn.execute "{ call nokglob.p_init_session_globals() }"
                    }
