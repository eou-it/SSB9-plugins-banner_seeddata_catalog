...
public class NorcmntDML {
    ...
    public NorcmntDML(InputData connectInfo, Sql conn, Connection connectCall, xmlData, List columns, List indexColumns, Batch batch,
                      def deleteNode) {
    ...
    def processNorcmnt() {
        ...
        if (transactionId) {
            apiData.NORCMNT_TRANSACTION_NO[0].setValue(transactionId?.toString())
            isValid = true
        }
    ...
    private def fetchTransactionId(paramList) {
        ...
        try {
            ...
            if (connectInfo.showErrors) println("Error while generating transaction Id in NorcmntDML for ${connectInfo.tableName}. $e.message")
        }
