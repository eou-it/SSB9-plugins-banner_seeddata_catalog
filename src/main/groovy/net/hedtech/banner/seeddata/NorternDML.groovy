...
public class NorternDML {
    ...
    public NorternDML(InputData connectInfo, Sql conn, Connection connectCall, xmlData, List columns, List indexColumns, Batch batch,
                      def deleteNode) {
    ...
    def processNortern() {
        ...
        if (transactionId) {
            apiData.NORTERN_TRANSACTION_NO[0].setValue(transactionId?.toString())
            isValid = true
        }
    ...
    private def fetchTransactionId(paramList) {
        ...
        try {
            ...
            if (connectInfo.showErrors) println("Error while generating transaction Id in NorternDML for ${connectInfo.tableName}. $e.message")
        }
