...
public class NorroutDML {
    ...
    public NorroutDML(InputData connectInfo, Sql conn, Connection connectCall, xmlData, List columns, List indexColumns, Batch batch,
                      def deleteNode) {
    ...
    def processNorrout() {
        ...
        if (transactionId) {
            apiData.NORROUT_TRANSACTION_NO[0].setValue(transactionId?.toString())
            isValid = true
        }
    ...
    private def fetchTransactionId(paramList) {
        ...
        try {
            ...
            if (connectInfo.showErrors) println("Error while generating transaction Id in NorroutDML for ${connectInfo.tableName}. $e.message")
        }
