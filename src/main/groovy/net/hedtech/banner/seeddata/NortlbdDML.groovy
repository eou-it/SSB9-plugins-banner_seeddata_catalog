...
public class NortlbdDML {
    ...
    public NortlbdDML(InputData connectInfo, Sql conn, Connection connectCall, xmlData, List columns, List indexColumns, Batch batch,
                      def deleteNode) {
    ...
    def processNortlbd() {
        ...
        if (transactionId) {
            apiData.NORTLBD_TRANSACTION_NO[0].setValue(transactionId?.toString())
            isValid = true
        }
    ...
    private def fetchTransactionId(paramList) {
        ...
        try {
            ...
            if (connectInfo.showErrors) println("Error while generating transaction Id in NortlbdDML for ${connectInfo.tableName}. $e.message")
        }
