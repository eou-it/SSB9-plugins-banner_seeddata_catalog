...
INSERT INTO testTables VALUES ( clob_sequence.nextval,'SORLFOS','select '''  || :bannerid || '''  bannerid,	SORLFOS.*	FROM 	SORLFOS, sorlcur	where 	SORLFOS_pidm = gb_common.f_get_pidm(''&&BID'') and sorlcur_pidm = sorlfos_pidm and sorlcur_seqno = sorlfos_lcur_seqno and sorlcur_lmod_code = sb_curriculum_str.f_outcome');
