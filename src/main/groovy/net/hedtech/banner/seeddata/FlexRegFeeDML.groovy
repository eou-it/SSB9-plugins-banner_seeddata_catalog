...
public class FlexRegFeeDML {
    ...
    def processDiscount() {
        ...
        def flexCheck
        conn.call("{$Sql.VARCHAR = call genutil.product_installed('SF')}") {flex ->
            flexCheck = flex
        }
        if (flexCheck == "Y") {
            // banner ID
            ...
            if (apiData?.CRN?.text() && apiData?.TERM_CODE?.text() && bannerid)
                ...
                ----------------------------------------
                    ...
                    delete FLEXREG.SFKRITEM where sfkritem_crn = crn and sfkritem_term_code = term_code and sfkritem_pidm = pidm;
                ...
                --  cart status values
                    ...
                    insert into FLEXREG.SFKRITEM (SFKRITEM_CID,SFKRITEM_CRN,SFKRITEM_TERM_CODE, SFKRITEM_STATUS,
                      ...
                      SFKRITEM_DATA_ORIGIN,SFKRITEM_RSTS_CODE,SFKRITEM_GMOD_CODE,SFKRITEM_CREDIT_HR,SFKRITEM_LEVL_CODE,SFKRITEM_VERSION, SFKRITEM_PIDM)
                    values (cid,crn,term_code,cart_item_status_registered,null,null,null,null,sysdate,'FLEXREG_USER','TEST','RW',null,null,null,0, pidm);

                    -- Create a class fee 
                    INSERT INTO FLEXREG.sffrfees (sffrfees_cid,sffrfees_term_code,sffrfees_crn,sffrfees_detail_code,
                         sffrfees_amount,sffrfees_note,sffrfees_type_ind,sffrfees_activity_date,sffrfees_user_id,sffrfees_data_origin, SFFRFEES_PIDM)
                    values(cid, term_code, crn,tuition_tbbdetc_code,500,null,'F',sysdate,'FLEXREG_USER','TEST', pidm);

                    -- Create a coupon
                    INSERT INTO FLEXREG.sffrfees (sffrfees_cid,sffrfees_term_code,sffrfees_crn,sffrfees_detail_code,
                         sffrfees_amount,sffrfees_note,sffrfees_type_ind,sffrfees_activity_date,sffrfees_user_id,sffrfees_data_origin, SFFRFEES_PIDM)
                    values(cid, term_code, crn,coupon_tbbdetc_code,-50,'Coupon 10% off','C',sysdate,'FLEXREG_USER','TEST', pidm);

                    -- Create a discount 
                    INSERT INTO FLEXREG.sffrfees (sffrfees_cid,sffrfees_term_code,sffrfees_crn,sffrfees_detail_code,
                         sffrfees_amount,sffrfees_note,sffrfees_type_ind,sffrfees_activity_date,sffrfees_user_id,sffrfees_data_origin, SFFRFEES_PIDM)
                            values(cid, term_code, crn,discount_tbbdetc_code,-100,'10% early bird','D',sysdate,'FLEXREG_USER','TEST', pidm);
                    COMMIT;
                END;
                """)
