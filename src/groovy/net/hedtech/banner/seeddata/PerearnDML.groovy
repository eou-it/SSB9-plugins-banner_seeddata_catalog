...
/**
 * Creates earnings data for timesheets
 */
class PerearnDML {
    ...
    public PerearnDML(InputData connectInfo, Sql conn, Connection connectCall, xmlData, List columns, List indexColumns, Batch batch,
                      def deleteNode) {
        this.conn = conn
    ...
    def processPerearn() {
        ...
        try {
            ...
            if (connectInfo.showErrors) {
                println "Could not select Pidm in PerearnDML for Banner ID ${apiData.BANNERID.text()} from SPRIDEN. $e.message"
            }
        }
        ...
        if (jobsSeqno) {
            ...
            apiData.children().each() { fields ->
                ...
                if (fields.name() == 'PEREARN_JOBS_SEQNO') {
            ...
            if (connectInfo.debugThis) {
                println "Perearn record Job Seqno ${jobsSeqno.toString()} "
            }
    ...
    def findJobsSeqno(apiData, pidm) {
        ...
        try {
            ...
            if (connectInfo.showErrors) {
                println "Could not select jobs seqno in PerearnDML for Banner ID ${apiData.BANNERID.text()}. $e.message"
            }
        }
