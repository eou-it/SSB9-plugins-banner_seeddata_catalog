...
/**
 * Creates time in and out data for timesheets
 */
class PertitoDML {
    ...
    public PertitoDML(InputData connectInfo, Sql conn, Connection connectCall, xmlData, List columns, List indexColumns, Batch batch,
                      def deleteNode) {
        this.conn = conn
    ...
    def processPertito() {
        ...
        try {
            ...
            if (connectInfo.showErrors) {
                println "Could not select Pidm in PertitoDML for Banner ID ${apiData.BANNERID.text()} from SPRIDEN. $e.message"
            }
        }
        ...
        if (jobsSeqno) {
            ...
            apiData.children().each() { fields ->
                ...
                if (fields.name() == 'PERTITO_JOBS_SEQNO') {
            ...
            if (connectInfo.debugThis) {
                println "Pertito record Job Seqno ${jobsSeqno.toString()} "
            }
    ...
    def findJobsSeqno(apiData, pidm) {
        ...
        try {
            ...
            if (connectInfo.showErrors) {
                println "Could not select jobs seqno in PertitoDML for Banner ID ${apiData.BANNERID.text()}. $e.message"
            }
        }
