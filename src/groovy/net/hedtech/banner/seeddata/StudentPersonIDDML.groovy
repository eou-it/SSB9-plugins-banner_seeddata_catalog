...
public class StudentPersonIDDML {
    ...
    def deleteData() {
        ...
        if (doesTableExist("SHRGRID")) deleteData("SHRGRID", "delete SHRGRID where  SHRGRID_RGID_GUID IN  ( SELECT SFRRGID_GUID SFRRGID where SFRRGID_pidm = ? ) ")
        if (doesTableExist("SFRGSID")) deleteData("SFRGSID", "delete SFRGSID where SFRGSID_RGID_GUID IN ( SELECT SFRRGID_GUID SFRRGID where SFRRGID_pidm = ? ) ")
