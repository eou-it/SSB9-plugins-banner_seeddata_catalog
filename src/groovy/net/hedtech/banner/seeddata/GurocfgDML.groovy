...
class GurocfgDML {
    ...
    public GurocfgDML(InputData connectInfo, Sql conn, Connection connectCall, xmlData, List columns, List indexColumns, Batch batch,
                      def deleteNode) {
    ...
    def parseXmlData() {
        ...
        this.configSeqNum = rule.GUROCFG_SURROGATE_ID.text()
        this.configType = rule.GUROCFG_TYPE.text()
        this.appId = rule.GUROCFG_GUBAPPL_APP_ID.text()
        this.configValue = rule.GUROCFG_VALUE.text()
        this.appName = rule.GUBAPPL_APP_NAME.text()
        this.version = rule.GUROCFG_VERSION.text()
    ...
    def insertGuroCfgData() {
        String appSql = """select GUBAPPL_APP_ID as appID from gubappl  where UPPER(GUBAPPL_APP_ID)= ?"""
        def params = [this.appId.toUpperCase()]
        ...
        if (this.appId) {
            String configSql = """select GUROCFG_SURROGATE_ID as configSeqValue from gurocfg  where GUROCFG_GUBAPPL_APP_ID= ? and UPPER(GUROCFG_NAME) = ?  and UPPER(GUROCFG_TYPE) = ?"""
