...
public class GeneralActionItemDML {
    ...
    def bannerid, newOracleId, oracleId, newClass, generalClass, objectName, objectRole, readonly
    ...
    def processData() {
        ...
        if (apiData.CREATE_USER?.text()) {
            this.newOracleId = apiData.CREATE_USER.text()
            this.readonly = apiData.READONLY.text()
        }
        if (apiData.GENERAL_CLASS?.text()) {
            this.oracleId = apiData.ORACLEID.text()
            this.generalClass = apiData.GENERAL_CLASS.text()
        }
        if (apiData.OBJECT?.text()) {
            this.oracleId = apiData.ORACLEID.text()
            this.objectName = apiData.OBJECT.text()
            this.objectRole = apiData.OBJECT_ROLE.text()
        }
        if (apiData.NEW_CLASS?.text()) {
            this.newClass = apiData.NEW_CLASS.text();
        }
