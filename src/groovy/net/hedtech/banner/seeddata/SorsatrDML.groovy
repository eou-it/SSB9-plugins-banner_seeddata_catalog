...
public class SorsatrDML {
    ...
    public SorsatrDML(InputData connectInfo, Sql conn, Connection connectCall, xmlData, List columns, List indexColumns, Batch batch, def deleteNode) {
    ...
    def processDelete() {
        String schDel = ""
        schDel = """DELETE FROM sorsatr  WHERE sorsatr_data_origin = 'GRAILS' """
        def tableName = "SORSATR"
    ...
    private def deleteData(String tableName, String sql) {
        try {
            ...
            if (connectInfo.showErrors) {
                println "Problem executing  ${tableName} from sorsatrDML.groovy: $e.message"
                println "${sql}"
            }
        }
    }
    ...
    /**
     * Process the sorsatr records.  Use the dynamic insert process but update the ssrmeet_surrogate_id before the insert
     *
     */
    ...
    def processSorsatr() {
        ...
        try {
            ...
            if (connectInfo.showErrors) {
                println "Could not select ID in SorsatrDML,  [${apiData.SSRMEET_TERM_CODE.text()}, ${apiData.SSRMEET_CRN.text()}, ${apiData.SSRMEET_CATAGORY.text()}, " +
                                "${apiData.SSRMEET_START_DATE.text()}, ${apiData.SSRMEET_END_DATE.text()}, ${apiData.SSRMEET_BEGIN_TIME.text()}, " +
        ...
        if (surrogateId) {
            ...
            if (connectInfo.tableName == "SORSATR") {
                if (apiData.SORSATR_SURROGATE_ID_SSRMEET.text().toInteger() != surrogateId.ID) {
                    apiData.SORSATR_SURROGATE_ID_SSRMEET[0].setValue(surrogateId.ID.toString())
                }
...
//        println(xmlRecNew)
            ...
            connectInfo.tableUpdate('SORSATR', 0, 0, 0, 1, 0)
        }
