        ...
        try {
            final String apiQuery =
                    "DECLARE\n" +
                            ...
                            "          p_uoms_code => 'EA', p_unit_price => j*5, p_qty => j*2, p_disc_amt => 0, p_tax_amt => 0, p_addl_chrg_amt => 0, p_tgrp_code => 'PA', p_ship_code => 'BIO', p_reqd_date => lv_trans_date+10, p_print_text => 'N', p_noprint_text => 'N', p_liq_amt => 0, p_vend_ref_num => NULL, p_agre_code =>NULL, p_susp_ind => 'N', p_closed_ind => NULL, p_cancel_ind => NULL, p_cancel_date => NULL, p_ttag_num => NULL, p_text_usage =>'S', p_convert_unit_price => NULL, p_convert_disc_amt => NULL, p_convert_tax_amt => NULL, p_convert_addl_chrg_amt => NULL, p_ext_amt => NULL, p_bo_remain_bal => NULL, p_desc_chge_ind => NULL, p_rowid =>lv_comm_rowid, p_data_changed => lv_comm_rowid);\n" +
                            ...
                            "              p_convert_addl_chrg_amt => NULL, p_rowid => lv_acctg_rowid, p_data_changed => NULL);\n" +
                            "            EXCEPTION\n" +
                            "            WHEN OTHERS THEN\n" +
                            "              p_error_msg_out := gb_common_strings.f_append_error(p_error_msg_out, ltrim(SQLERRM,1500));\n" +
                            "              lv_fatal_error  := true;\n" +
                            ...
                            "            , p_rowid => lv_acctg_rowid, p_data_changed => NULL);\n" +
                            "          EXCEPTION\n" +
                            "          WHEN OTHERS THEN\n" +
                            "            p_error_msg_out := gb_common_strings.f_append_error(p_error_msg_out, ltrim(SQLERRM,1500));\n" +
                            "            lv_fatal_error  := true;\n" +
