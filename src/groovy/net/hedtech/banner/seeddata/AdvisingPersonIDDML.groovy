...
/**
 * General Person ID DML for Advisors
 * Save the PIDM in the InputData.saveAdvisingPidm and delete existing advising data
 */

public class AdvisingPersonIDDML {
    def spriden_pidm
    def spriden_id
    ...
    public AdvisingPersonIDDML(InputData connectInfo, Sql conn, Connection connectCall, xmlData) {
    ...
    def processAdvising() {
        ...
        try {
            ...
            if (connectInfo.showErrors) {
                println "Could not select ID in AdvisingPersonIDDML,  ${apiData.SPRIDEN_ID.text()}  from SPRIDEN. $e.message"
            }
        }
        if (cntSpriden == 0) {
            def newSpriden = new PersonIDDML(connectInfo, conn, connectCall, xmlData)
            connectInfo.saveAdvisingPidm = newSpriden.PIDM

        }
        else {
            connectInfo.saveAdvisingPidm = pidm
        }
    ...
    def deleteData() {
        ...
        String selectSql = """select srbrecr_term_code, srbrecr_admin_seqno   from srbrecr where srbrecr_pidm = ?
               order by 1,2"""
        try {
            conn.eachRow(selectSql, [connectInfo.saveAdvisingPidm]) {

                conn.call("{call sb_recruit.p_delete(p_pidm => ?,p_term_code => ?, p_admin_seqno =>? ) }",
                          [connectInfo.saveAdvisingPidm.toInteger(),
            ...
            if (connectInfo.showErrors) {
                println "Problem executing select or delete for table SRBRECR from AdvisingPersonIDDML.groovy: $e.message"
            }
        }
        ...
        try {
            ...
            if (connectInfo.showErrors) {
                println "Problem executing select or delete for table SARADAP from AdvisingPersonIDDML.groovy: $e.message"
            }
        }
        ...
        try {
            conn.eachRow(selectSql, [connectInfo.saveAdvisingPidm]) {
                conn.call("""{call sb_learner.p_delete(p_pidm => ?,
                            p_term_code_eff => ?) }""",
                          [connectInfo.saveAdvisingPidm.toInteger(), it.sgbstdn_term_code_eff])
            }
        }
        catch (e) {
            connectInfo.tableUpdate("SGBSTDN", 0, 0, 0, 1, 0)
            if (connectInfo.showErrors) {
                println "Problem executing select or delete for table SGBSTDN from AdvisingPersonIDDML.groovy: $e.message"
            }
        }
        ...
        try {
            conn.eachRow(selectSql, [connectInfo.saveAdvisingPidm]) {
                conn.call("{call sb_learneroutcome.p_delete(p_pidm => ?, p_seq_no => ? ) }",
                          [connectInfo.saveAdvisingPidm.toInteger(), it.shrdgmr_seq_no.toInteger()])
            }
        }
        catch (e) {
            connectInfo.tableUpdate("SHRDGMR", 0, 0, 0, 1, 0)
            if (connectInfo.showErrors) {
                println "Problem executing select or delete for table SHRDGMR from AdvisingPersonIDDML.groovy: $e.message"
            }
        }
        ...
        def terms = conn.rows(selectTerms, [connectInfo.saveAdvisingPidm])
     ...
     """
        ...
        try {
            ...
            if (connectInfo.showErrors) {
                println "Problem executing enrollment count update ${connectInfo.saveAdvisingPidm} from AdvisingPersonIDDML.groovy: $e.message"
            }
        }
        if (connectInfo.saveThis) {
            conn.execute "{ call gb_common.p_commit() }"
    ...
    def deleteData(String tableName, String sql) {
        try {
            ...
            int delRows = conn.executeUpdate(sql, [connectInfo.saveAdvisingPidm.toInteger()])
            connectInfo.tableUpdate(tableName, 0, 0, 0, 0, delRows)
        }
        catch (Exception e) {
            if (connectInfo.showErrors) {
                println "Problem executing delete for person ${connectInfo.saveAdvisingPidm} from AdvisingPersonIDDML.groovy: $e.message"
                println "${sql}"
            }
        }
    }
    ...
    private def updateTermData(String tableName, String sql, String term, String crn) {
        try {
            ...
            if (connectInfo.showErrors) {
                println "Problem executing update term data ${connectInfo.saveAdvisingPidm} ${term} ${crn} from AdvisingPersonIDDML.groovy: $e.message"
                println "${sql}"
            }
        }
